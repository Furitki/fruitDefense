...
namespace FruitDefense.Editor
{
    public static class BattleSnapshotV1Smoke
    {
        ...
        private static void ValidateBananaContinuation(CompiledBattleContentCatalog catalog)
        {
            ...
            simulation.Step();
            var cooldownTicks = BattleSkillTiming.SecondsToTicks(999f);
            foreach (var runtime in simulation.State.Plants[0].SkillRuntimes)
                if (runtime.SkillId == BattleContentIds.Skills.BananaAttack)
                    runtime.CooldownTicks = cooldownTicks;
            simulation.State.Plants[0].AttackCooldown = BattleSkillTiming.TicksToSeconds(cooldownTicks);
