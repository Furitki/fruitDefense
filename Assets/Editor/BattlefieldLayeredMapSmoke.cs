...
namespace FruitDefense.Editor
{
    public static class BattlefieldLayeredMapSmoke
    {
        private const string FixturePath =
            "Assets/Editor/Fixtures/battlefield-layered-pre-migration.json";
        ...
        {
            return new BattlefieldLayeredMapSource(source.SchemaVersion, source.MapId,
        ...
        private static string Masks(BattlefieldMapDefinition map, string surfaceId)
        {
            ...
            for (var x = 0; x <= map.GridWidth; x++)
                values[index++] = ((int)BattlefieldDualGridTerrain.ResolveMask(
                    map, x, y, surfaceId)).ToString();
