# outgame-home-activity-presentation Specification

## Purpose
Define the stable reference-approved Home, Activity, primary-action, and bottom-navigation presentation contract without turning concept pixels or unsupported reference data into runtime authority.
...
The Lobby Activity page SHALL present the starter activity title and finite description, one dedicated text-free reward hero illustration, two graphical reward tiles backed by the actual reward grants, an explicit claim state, and one leaf-green Claim primary action in that reading order.
