# plant-selection-inspection Specification

## Purpose
TBD - created by archiving change simplify-plant-selection-and-movement. Update Purpose after archive.
## Requirements
...
Drag targeting SHALL retain visual target highlights, but the floating text hint SHALL appear only for a legal plant merge and SHALL use a compact content-sized frame.
...
- **WHEN** a dragged plant is over a compatible same-kind, same-star plant and the merge is legal
- **THEN** a compact floating hint identifies the resulting star level
