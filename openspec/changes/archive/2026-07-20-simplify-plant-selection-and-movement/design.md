...
### Keep only the merge text cue

Drag target colors remain the immediate legality feedback for planting, movement, nursery return, and invalid destinations. The floating text frame is reserved for a legal merge because the resulting star level is the only target outcome that benefits from extra copy. Its width follows the merge text within a bounded compact range instead of spanning most of the battlefield.
...
- [Removing routine drag copy makes targets ambiguous] -> Retain the existing legal, merge, cancel, and invalid target highlight colors while suppressing only the floating text frame.
