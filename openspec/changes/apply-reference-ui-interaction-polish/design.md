...
Static analysis of the reference APK shows that its transferable interaction language is comparatively small: high-frequency feedback around 0.10–0.20 seconds, restrained overshoot for important information, programmatic scale/offset/alpha sequences, cancellation before replay, and separate use of Animator/Spine for high-value authored moments. Its Lua/C# ownership split, overlapping button subclasses, and large family of atomic Tween classes are unsuitable for this project.
...
- Make reduced motion a first-class policy that renders the same semantic end states without travel, stagger, or overshoot.
...
The theme feedback block will add validated values for press scale, pop scale, reveal offset, reveal duration, stagger interval, and reduced-motion default. Reference values guide the initial range, but amplitudes are deliberately lower than the largest APK effects: routine controls stay near 0.96–1.12 scale and only settlement emphasis may reach approximately 1.16.
...
### 6. Reduced motion collapses travel, stagger, and overshoot
...
- [Too much overshoot makes the UI toy-like or noisy] → Centralize amplitude tokens, cap concurrent animated groups, and use the strongest pop only for result/resource emphasis.
